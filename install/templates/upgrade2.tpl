...
{foreach from=$settings.required item='test'}
	...
	{if isset($test->error_fragment)}
				<a class="external" rel="external" href="{$cms_upgrade_help_url}#{$test->error_fragment}">?</a>
	{/if}
			</td>
		</tr>
{/foreach}
...
<table class="legend" border="0">
	...
	<tbody>
		...
		<tr>
			<td class="col2">
				<img src="images/yellow.gif" alt="{if isset($caution)}{$caution}{/if}" title="{if isset($caution)}{$caution}{/if}"/>
			</td>
			<td>
