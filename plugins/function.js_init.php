...
function smarty_function_js_init($params, &$template){
	...
	var lang = getTranslate('<?php echo CmsNlsOperations::get_current_language() ?>');
	$(document).ready(function(){
		...
		$(".firstStep input[type='button']").click(function(){
			if(validFirstStep && validFirstStepAjax && !$(this).hasClass("disabled") && isMoreThanTenEur(this)){
				...
				if($(this).attr("id") == "card" && $("#sell-online-card").val() == 0){
					$(".sell-online-card-notvalid").css("display","block").html(lang.vacsieodnuly);
				}
				if($(this).attr("id") == "buy" && $("#buy-bitcoin").val() == 0){
					$(".buy-bitcoin-notvalid").css("display","block").html(lang.vacsieodnuly);
				}
			}
		});
		...
		$(".thirdStep input[type='button']").click(function(){
			...
			if(action == "backToSecondStep"){
				...
				vpb_submit_captcha('<?php echo CmsNlsOperations::get_current_language(); ?>');
			}
		});
	});
	...
	function changeInputValueFirstStep(object,type){
		...
		if(newVal > 0){
			...
			showValidationFirstStep(newVal,false,lang.vacsieodnuly,((isSellSide) ? "sell-online-card" : "buy-bitcoin"),isBitcoin);
			$("#"+inputId+"-info").val(0);
			ajaxCounting = false;
			return false;
		}
	...
	function sendData(){
		...
		$.ajax({
			...
			data: {
				...
				"lang" : '<?php echo CmsNlsOperations::get_current_language() ?>'
			},
			beforeSend: function() 
			{
				//sem dat gifko
	...
	function showOrderFieldByTransaction(typeOfTransaction){
		...
		if(typeOfTransaction == "buy"){
			...
			$(".typeOfServiceTitle").html(lang.predatbitcoin);
			$(".sposobPlatby").hide();
			$(".captcha h5").html("2. " + lang.vyplncaptchu);
		}else{
	...
	function isMoreThanTenEur(object){
		if($(object).parents().hasClass("lavy-predaj")){
			if($("#sell-online-card").val() < 10){
				...
				$(".sell-online-card-notvalid").html(lang.minimalnedesat);
				return false;
			}else{
				$(".sell-online-card-notvalid").hide();
				$(".sell-online-card-notvalid").html(lang.niejespravne);
				return true;
			}
			...
			if($("#buy-bitcoin-info").val() < 10){
				...
				$(".buy-bitcoin-notvalid").html(lang.minimalnedesat);
				return false;
			}else{
				$(".buy-bitcoin-notvalid").hide();
				$(".buy-bitcoin-notvalid").html(lang.niejespravne);
				return true;
			}
		}
	}
	function getTranslate(actualLang){
		var translate = {"sk": 
                        	{	
        						"prvykrok": "Chyba pri validácii prvého kroku", 
        						"vacsieodnuly": "Prosím vyplnte hodnotu väčšiu od nuly", 
        						"druhykrok": "Chyba pri validácii druhého kroku",
        						"odoslobjednavky": "Chyba pri odoslaní objednávky", 
        						"prevysuje": "Výsledná suma prevyšuje hodnotu v ",
        						"kdispozicii": " ,ktorá je k dispozícii", 
        						"vypocetsumy": "Chyba pri výpočte sumy",
        						"obnovakurzov": "Chyba pri obnovení kurzov", 
        						"kupitbitcoin": "Kúpiť bitcoin", 
        						"predatbitcoin": "Predať bitcoin",
        						"vyplncaptchu": "Vyplnte captchu",
        						"predavate": "Predávate", 
        						"cena": "Cena", 
        						"nakup": "Nákup",
        						"minimalnedesat": "Hodnota eur musí byť minimálne 10", 
        						"niejespravne": "Pole nie je vyplnené správne"
            				},
                         "en" : 
                            {
            					"prvykrok": "Error in first step of validating", 
        						"vacsieodnuly": "Please fill value greater than zero", 
        						"druhykrok": "Error in second step of validating",
        						"odoslobjednavky": "Error while sending the order", 
        						"prevysuje": "The resulting amount exceeds the value of the ",
        						"kdispozicii": " ,which is available", 
        						"vypocetsumy": "Error in calculating the amount",
        						"obnovakurzov": "Error when restoring courses", 
        						"kupitbitcoin": "Buy Bitcoin",
        						"predatbitcoin": "Sell bitcoin", 
        						"vyplncaptchu": "Fill CAPTCHA",
        						"predavate": "Sell", 
        						"cena": "Price", 
        						"nakup": "Buy",
        						"minimalnedesat": "The value must be at least  10eur", 
        						"niejespravne": "Field isn´t filled out correctly"
                            }
                         };
		return ((actualLang == "sk_SK") ? translate.sk : translate.en);
	}
	</script>
	<?php
}
