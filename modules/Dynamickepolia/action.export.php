...
if (!$this->CheckAccess('Manage Dynamickepolia')) {
...
if (MCFTools::IsModuleActive('CMSExcel'))
{
	$params = array(
		'creator' => 'M&C Factory',
		'title' => 'Dynamickepolia export',
		'subject' => 'Dynamickepolia export',
		);
	
	$stack = cms_utils::get_module('CMSExcel')->buildExcel($datas,$params);
	
	$link = cms_utils::get_module('CMSExcel')->getExcelLink($stack,array('filename' => 'Dynamickepolia_export_'.date('Y-m-d_H-i-s').'.xls'));
