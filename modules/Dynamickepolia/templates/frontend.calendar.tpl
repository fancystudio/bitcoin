			...
			<tr>
				...
				<td class="{if $day.current}current {/if}{if $day.today}today {/if}{if $day.events|@count} has-events{/if}">
					...
					{* Alternative: Link to all events of the current date }
						{if $day.events|@count}
							<a href="{Dynamickepolia action="url_for" maction="default" calendar=1 start_date=$day.date|date_format:"%Y-%m-%d" end_date=$day.date|date_format:"%Y-%m-%d"}">{$day.date|date_format:"%e"}</a>
						{else}
