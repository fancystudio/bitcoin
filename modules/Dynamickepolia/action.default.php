...
DynamickepoliaObject::globalFrontendFilters($c, $params);
...
elseif (
	...
	&& 
	isset($_SESSION['modules']['Dynamickepolia']['calendar']['scope'])
	&&
...
{
	 $calendar = new MCFCalendar($_SESSION['modules']['Dynamickepolia']['calendar']['scope']);
   $c->add('start_date', date('Y/m/d', $calendar->end_date), MCFCriteria::LESS_EQUAL);
   $c->add('end_date', date('Y/m/d', $calendar->start_date), MCFCriteria::GREATER_EQUAL);
}
...
if(is_object($modulextender))  {
  if ($modulextender->isXtendedModule('Dynamickepolia')) {
    $mxfilters = array();
...
{
  $tree = DynamickepoliaObject::buildTree($items);
  $this->smarty->assign('dynamickepolia_tree', $tree); 
}

if (isset($params['limit'])) {
	$c->setOffset(0);
	$c->setLimit(0);
	$total = DynamickepoliaObject::doCount($c);
...
if (isset($params['var'])) {
    ...
    $this->smarty->assign('dynamickepolia_params', $paramsobj);
