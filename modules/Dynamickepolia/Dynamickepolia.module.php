...
class Dynamickepolia extends CMSModule {
  ...
  public function GetName() {               return 'Dynamickepolia';  }
  public function GetObjectName() {         return 'DynamickepoliaObject';  }
  public function GetFriendlyName() {       return 'Dynamickepolia';  }
  ...
  public  function CheckAccess($permission = 'Manage Dynamickepolia') {    return $this->CheckPermission($permission);  }
  ...
  public function Install() {
    ...
    $sqlarray = $dict->CreateTableSQL(cms_db_prefix().'module_dynamickepolia', implode(',',$fields));
    $dict->ExecuteSQLArray($sqlarray);
    $db->CreateSequence(cms_db_prefix().'module_dynamickepolia_seq');
    ...
    //@mkdir($config['root_path'].'/uploads/Dynamickepolia');
    $this->CreateEvent('ContentEditPost');
    cms_utils::get_module('MCFactory')->AddEventHandler($this->getName(), 'ContentEditPost', false);
    $this->CreatePermission('Manage Dynamickepolia', 'Manage Dynamickepolia');  
    $this->CreatePermission('Admin Dynamickepolia', 'Admin Dynamickepolia');  
  ...
  public function Upgrade($oldversion, $newversion)  {
    ...
    switch(true) {
      ...
      case version_compare($oldversion, '2.9.16', '<'):
          $sqlarray = $dict->AddColumnSQL(cms_db_prefix() . 'module_dynamickepolia', 'user_id I');
          $dict->ExecuteSQLArray($sqlarray);
      case version_compare($oldversion, '2.12.1', '<'):
          $sqlarray = $dict->AddColumnSQL(cms_db_prefix() . 'module_dynamickepolia', 'mcfi_created_timestamp I');
          $dict->ExecuteSQLArray($sqlarray);
          $sqlarray = $dict->AddColumnSQL(cms_db_prefix() . 'module_dynamickepolia', 'mcfi_updated_timestamp I');
          $dict->ExecuteSQLArray($sqlarray);
      case version_compare($oldversion, '3.1.8', '<'):
          $sqlarray = $dict->AddColumnSQL(cms_db_prefix() . 'module_dynamickepolia', 'send_update_immediately I');
          $dict->ExecuteSQLArray($sqlarray);
  ...
  public function Uninstall() {
    $db = cms_utils::get_db();
    $dict = NewDataDictionary($db);
    $sql = $dict->DropTableSQL(cms_db_prefix().'module_dynamickepolia');
    $dict->ExecuteSQLArray($sql);
    $db->DropSequence(cms_db_prefix().'module_dynamickepolia_seq');
  ...
  function InitializeGlobal() {  
    ...
    // $this->RegisterRoute('/dynamickepolia\/(?P<item_id>[0-9]+)(\/.*?)?$/', array('action' => 'detail', 'returnid' => cmsms()->GetContentOperations()->GetDefaultPageID()));
    $this->RegisterRoute('/dynamickepolia\/(?P<item_id>[0-9]+)\/(?P<returnid>[0-9]+)(\/.*?)?$/', array('action' => 'detail'));
    $this->RegisterRoute('/dynamickepolia\/rss\/(?P<maction>[a-zA-Z0-9_-]+)(\/.*?)?$/', array('action' => 'rss', 'showtemplate' => 'false','returnid' => cmsms()->GetContentOperations()->GetDefaultPageID()));
    $this->RegisterRoute('/dynamickepolia\/api\/(?P<command>[a-zA-Z0-9_-]+)(\/.*?)?$/', array('action' => 'api', 'showtemplate' => 'false','returnid' => cmsms()->GetContentOperations()->GetDefaultPageID()));
    
    $this->RegisterRoute('/dynamickepolia\/download\/(?P<item_id>[0-9]+)\/(?P<field>[a-zA-Z0-9_-]+)(\/.*?)?$/', 
      array(
      'action' => 'download', 
      'showtemplate' => 'false',
  ...
  function createLink($id, $action, $returnid='', $contents='', $params=array(), $warn_message='', $onlyhref=false, $inline=false, $addttext='', $targetcontentonly=false, $prettyurl='',  $withslash = false) {
    ...
    if (empty($prettyurl)) {
      if ($action == 'detail') {
        ...
        $prettyurl = 'dynamickepolia/' . $item_id . '/' . $returnid;
  ...
  function deindex($item) {
    $search = $this->GetModuleInstance('Search');
    if ($search) {
      $search->DeleteWords('Dynamickepolia', $item->getId(), 'dynamickepolia_item');
    }
  }

  function SearchResult($returnid, $id, $attr = '') {
    $result = array();
    if ($attr == 'dynamickepolia_item') {
      $c = new MCFCriteria();
      $c->add('id', $id);
      $c->add('published', 1);
  ...
  */
  ...
  {
    $item = DynamickepoliaObject::retrieveByPk($item_id);
  ...
  {
    // DEPRECATED
    DynamickepoliaObject::buildFiltersCriteria($c, $filters);
  }
  ...
  public static function getTitleLabel()
  {
	...
	{
		return DynamickepoliaObject::doSelect($c);
	}
}
