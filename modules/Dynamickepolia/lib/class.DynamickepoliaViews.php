...
public static function adminItems($module,$id,$returnid,$view,$items,$parent_id=null)
{
	...
	// Preview code
	if (!isset($_SESSION['Dynamickepolia']['preview_key']))
	{
		$_SESSION['Dynamickepolia']['preview_key'] = base64_encode(time());
	}
	...
	foreach ($items as $item) {
		...
		$rows[$item->getId()] = array(
		  ...
		  'parent_id' => $item->getParentId(),
			...
			'previewlink' => $module->CreateLink($id, 'detail', $module->getPreference('default_page'), cmsms()->get_variable('admintheme')->DisplayImage('icons/system/view.gif', $module->lang('preview_s',$item->getTitle()), '', '', 'systemicon'), array('preview_key' => $_SESSION['Dynamickepolia']['preview_key'], 'item_id' => $item->getId()), '', false,false,' target="_new"', true)
  ...
  if(class_exists('MX_Relation'))
  {
	...
	if (is_null($parent_id))
	{
		...
		DynamickepoliaObject::buildFiltersWidgets($form, $currentModuleFilters);		
...
public static function createForm($module,&$form,&$item,$params=array())
{
  $config = cms_utils::get_config();
        ...
        if(!isset($params['frontend'])) {        	  $form->getFieldset('default---default')->setWidget('pole1', 'text', array(
  		'label' => 'pole1', 
  		'object' => &$item, 
  		'size' => 50
  		  		));
  	    }	      if(!isset($params['frontend'])) {        	  $form->getFieldset('default---default')->setWidget('pole2', 'text', array(
  		'label' => 'Pole2', 
  		'object' => &$item, 
  		'size' => 50
  		  		));
