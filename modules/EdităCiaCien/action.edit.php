...
$view = new MCFListView('EditãCiaCien', $params);



if (isset($params['item_id']) && !empty($params['item_id'])) {
  $item = EditãCiaCienObject::getById($params['item_id']);
} else {
  $item = new EditãCiaCienObject();
...
$form = new CMSForm('EditãCiaCien', $id,'edit',$returnid);
...
EditãCiaCienViews::createForm($this,$form,$item);
...
// SUB MODULES SECTION
$c = new MCFCriteria();
$c->add('parent_module', 2, MCFCriteria::EQUAL);
...
if (is_object($modulextender) && $modulextender->isXtendedModule($this->getName())) {  
  $this->smarty->assign('xtended_form', $modulextender->XtendModuleForm('EditãCiaCien', $item->getId(), $id));
} else {
