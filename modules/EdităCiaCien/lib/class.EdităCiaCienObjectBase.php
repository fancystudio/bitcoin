...
class EditãCiaCienObjectBase extends MCFModuleObject {

  protected $childrens;
  protected $level;

  const MODULE_NAME = 'EditãCiaCien';
  const MODULE_OBJECT_NAME = 'EditãCiaCienObject';

  const DB_NAME = 'module_editãciacien';
  const DB_ITEM = 'editãciacien';
  const UPLOADS_RELATIVE_URL = '/uploads/Modules/EditãCiaCien';

  protected static $fields = array('title',  'parent_id',  'parent_item',  'order_by', 'published', 'send_update_immediately', 'btc_k_dispozci_na_predaj',   'EUR_k_dispozci_na_nkup',   'mara_v_Percent_na_predaj',   'mara_v_Percent_na_nkup',    'full_text_search'  );
  ...
  public function getParentItemObject() {
    return EditãCiaCienObject::doSelectOne(array('where' => array('id' => $this->parent_item)));
  }
