...
EditãCiaCienObject::globalFrontendFilters($c, $params);
...
elseif (
	...
	&& 
	isset($_SESSION['modules']['EditãCiaCien']['calendar']['scope'])
	&&
...
{
	 $calendar = new MCFCalendar($_SESSION['modules']['EditãCiaCien']['calendar']['scope']);
   $c->add('start_date', date('Y/m/d', $calendar->end_date), MCFCriteria::LESS_EQUAL);
   $c->add('end_date', date('Y/m/d', $calendar->start_date), MCFCriteria::GREATER_EQUAL);
}
...
if(is_object($modulextender))  {
  if ($modulextender->isXtendedModule('EditãCiaCien')) {
    $mxfilters = array();
...
{
  $tree = EditãCiaCienObject::buildTree($items);
  $this->smarty->assign('editciacien_tree', $tree); 
}

if (isset($params['limit'])) {
	$c->setOffset(0);
	$c->setLimit(0);
	$total = EditãCiaCienObject::doCount($c);
