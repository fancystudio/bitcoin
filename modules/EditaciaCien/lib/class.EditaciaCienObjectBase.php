...
class EditaciaCienObjectBase extends MCFModuleObject {

  protected $childrens;
  protected $level;

  const MODULE_NAME = 'EditaciaCien';
  const MODULE_OBJECT_NAME = 'EditaciaCienObject';

  const DB_NAME = 'module_editaciacien';
  const DB_ITEM = 'editaciacien';
  const UPLOADS_RELATIVE_URL = '/uploads/Modules/EditaciaCien';

  protected static $fields = array('title',  'parent_id',  'parent_item',  'order_by', 'published', 'send_update_immediately', 'EUR_k_dispozci_na_nkup',   'btc_k_dispozci_na_predaj',   'mara_v_Percent_na_nkup',   'mara_v_Percent_na_predaj',    'full_text_search'  );
  ...
  public function full_text_search()  {
    $text = $this->title;
                  $text .= ' ' . $this->EUR_k_dispozci_na_nkup;
                        $text .= ' ' . $this->btc_k_dispozci_na_predaj;
                        $text .= ' ' . $this->mara_v_Percent_na_nkup;
                        $text .= ' ' . $this->mara_v_Percent_na_predaj;
              return $text;
  }
  ...
  public function getParentItemObject() {
    return EditaciaCienObject::doSelectOne(array('where' => array('id' => $this->parent_item)));
  }
  ...
  /**
    * @param bool $parse
    */
  public function getEURKDispozciNaNkup($parse = false) {
    if ($parse) {
      return cms_module_ProcessTemplateFromData(new stdClass(), $this->EUR_k_dispozci_na_nkup);
    }
    return $this->EUR_k_dispozci_na_nkup;
  }

  public function setEURKDispozciNaNkup($value) {
        $this->__set('EUR_k_dispozci_na_nkup', $value);
  }
  ...
  /**
    * @param bool $parse
    */
  public function getBtcKDispozciNaPredaj($parse = false) {
    if ($parse) {
      return cms_module_ProcessTemplateFromData(new stdClass(), $this->btc_k_dispozci_na_predaj);
    }
    return $this->btc_k_dispozci_na_predaj;
  }

  public function setBtcKDispozciNaPredaj($value) {
        $this->__set('btc_k_dispozci_na_predaj', $value);
  }
  ...
  /**
    * @param bool $parse
    */
  public function getMaraVPercentNaNkup($parse = false) {
    if ($parse) {
      return cms_module_ProcessTemplateFromData(new stdClass(), $this->mara_v_Percent_na_nkup);
    }
    return $this->mara_v_Percent_na_nkup;
  }

  public function setMaraVPercentNaNkup($value) {
        $this->__set('mara_v_Percent_na_nkup', $value);
  }
  ...
  /**
    * @param bool $parse
    */
  public function getMaraVPercentNaPredaj($parse = false) {
    if ($parse) {
      return cms_module_ProcessTemplateFromData(new stdClass(), $this->mara_v_Percent_na_predaj);
    }
    return $this->mara_v_Percent_na_predaj;
  }

  public function setMaraVPercentNaPredaj($value) {
        $this->__set('mara_v_Percent_na_predaj', $value);
  }
  ...
  public function getSearchString() {
    $fields[] = $this->getTitle();
                $fields[] = $this->getEURKDispozciNaNkup();
                      $fields[] = $this->getBtcKDispozciNaPredaj();
                      $fields[] = $this->getMaraVPercentNaNkup();
                      $fields[] = $this->getMaraVPercentNaPredaj();
...
//    unset($array['full_text_search']);
  ...
  public function populateFromArray(array $params, $prefix = null) {
    ...
    if (isset($params[$prefix.'full_text_search']))
    {
      $this->setFullTextSearch($params[$prefix.'full_text_search']);
        ...
        if (isset($params[$prefix.'EUR_k_dispozci_na_nkup'])) {
      $this->setEURKDispozciNaNkup($params[$prefix.'EUR_k_dispozci_na_nkup']);
    }
        if (isset($params[$prefix.'btc_k_dispozci_na_predaj'])) {
      $this->setBtcKDispozciNaPredaj($params[$prefix.'btc_k_dispozci_na_predaj']);
    }
        if (isset($params[$prefix.'mara_v_Percent_na_nkup'])) {
      $this->setMaraVPercentNaNkup($params[$prefix.'mara_v_Percent_na_nkup']);
    }
        if (isset($params[$prefix.'mara_v_Percent_na_predaj'])) {
      $this->setMaraVPercentNaPredaj($params[$prefix.'mara_v_Percent_na_predaj']);
    }
      }
  ...
  public function toArray()
  {
    ...
    $array['published'] = $this->published;
        ...
        $array['EUR_k_dispozci_na_nkup'] = $this->getEURKDispozciNaNkup();
        $array['btc_k_dispozci_na_predaj'] = $this->getBtcKDispozciNaPredaj();
        $array['mara_v_Percent_na_nkup'] = $this->getMaraVPercentNaNkup();
        $array['mara_v_Percent_na_predaj'] = $this->getMaraVPercentNaPredaj();
  ...
  protected function insert($params = array()) {
    ...
    $query = 'INSERT INTO '.cms_db_prefix().self::DB_NAME . '
      SET id = ?,
        ...
        parent_id = ?,
        title = ?,
                EUR_k_dispozci_na_nkup = ?,
                btc_k_dispozci_na_predaj = ?,
                mara_v_Percent_na_nkup = ?,
                mara_v_Percent_na_predaj = ?,
                created_by = ?,
        created_at = NOW(),
    ...
    $db->Execute($query, array(
      ...
      $this->parent_id,
      $this->title,
            $this->EUR_k_dispozci_na_nkup,
            $this->btc_k_dispozci_na_predaj,
            $this->mara_v_Percent_na_nkup,
            $this->mara_v_Percent_na_predaj,
            $userid,
      time(),
      $userid,
      time(),
