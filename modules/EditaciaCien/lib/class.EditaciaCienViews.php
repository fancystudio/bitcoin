...
public static function adminItems($module,$id,$returnid,$view,$items,$parent_id=null)
{
	...
	// Preview code
	if (!isset($_SESSION['EditaciaCien']['preview_key']))
	{
		$_SESSION['EditaciaCien']['preview_key'] = base64_encode(time());
	}
	...
	foreach ($items as $item) {
		...
		$rows[$item->getId()] = array(
		  ...
		  'parent_id' => $item->getParentId(),
			...
			'previewlink' => $module->CreateLink($id, 'detail', $module->getPreference('default_page'), cmsms()->get_variable('admintheme')->DisplayImage('icons/system/view.gif', $module->lang('preview_s',$item->getTitle()), '', '', 'systemicon'), array('preview_key' => $_SESSION['EditaciaCien']['preview_key'], 'item_id' => $item->getId()), '', false,false,' target="_new"', true)
  ...
  if(class_exists('MX_Relation'))
  {
	...
	if (is_null($parent_id))
	{
		...
		EditaciaCienObject::buildFiltersWidgets($form, $currentModuleFilters);		
