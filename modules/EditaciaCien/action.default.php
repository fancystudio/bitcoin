...
EditaciaCienObject::globalFrontendFilters($c, $params);
...
elseif (
	...
	&& 
	isset($_SESSION['modules']['EditaciaCien']['calendar']['scope'])
	&&
...
{
	 $calendar = new MCFCalendar($_SESSION['modules']['EditaciaCien']['calendar']['scope']);
   $c->add('start_date', date('Y/m/d', $calendar->end_date), MCFCriteria::LESS_EQUAL);
   $c->add('end_date', date('Y/m/d', $calendar->start_date), MCFCriteria::GREATER_EQUAL);
}
...
if(is_object($modulextender))  {
  if ($modulextender->isXtendedModule('EditaciaCien')) {
    $mxfilters = array();
...
{
  $tree = EditaciaCienObject::buildTree($items);
  $this->smarty->assign('editaciacien_tree', $tree); 
}

if (isset($params['limit'])) {
	$c->setOffset(0);
	$c->setLimit(0);
	$total = EditaciaCienObject::doCount($c);
