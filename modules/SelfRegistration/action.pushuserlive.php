...
$this->_CreateFrontendUser( $tmpuser_id, $row['username'], $row['passsword'], '', FALSE );

$query = 'DELETE FROM '.cms_db_prefix().'module_selfreg_properties WHERE user = ?';
$db->Execute($query,array($tmpuser_id));
$query = 'DELETE FROM '.cms_db_prefix().'module_selfreg_users WHERE id = ?';
$db->Execute($query,array($tmpuser_id));
